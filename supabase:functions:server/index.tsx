...
// ========== SUPABASE KV STORE (Database-backed) ==========
const TABLE_NAME = 'kv_store_cf244566';
...
const kv = {
  ...
  get: async (key: string): Promise<any> => {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      if (error.code === 'PGRST116') return null; // Key doesn't exist
  ...
  getByPrefix: async (prefix: string): Promise<any[]> => {
    ...
    const { data, error } = await supabase
      ...
      .like('key', `${prefix}%`);
    if (error) {
      console.error('❌ KV getByPrefix error:', prefix, error);
      throw error;
    }
...
// Force redeploy - version 2.0.9 - FIXED CHECKOUT BUG (paymentIntentId return)
...
if (stripeSecretKey && stripeSecretKey.startsWith("sk_")) {
  ...
  console.warn("⚠️ STRIPE_SECRET_KEY not configured. Webhook functionality will be disabled.");
}
...
async function getOrCreateSubscriptionProduct(stripe: Stripe): Promise<string> {
  try {
    ...
    if (cachedProductId) {
      console.log("🔍 Validating cached product ID:", cachedProductId);
      
      // Validate that the product actually exists in Stripe
      try {
        const product = await stripe.products.retrieve(cachedProductId as string);
        if (product && product.active) {
          console.log("✅ Cached product is valid:", cachedProductId);
          return cachedProductId as string;
        } else {
          console.log("⚠️ Cached product is inactive, clearing cache");
          await kv.del("stripe:product:santa_subscription");
          await kv.del("stripe:price:santa_subscription");
        }
      } catch (error) {
        console.log("❌ Cached product doesn't exist, clearing cache:", error);
        await kv.del("stripe:product:santa_subscription");
        await kv.del("stripe:price:santa_subscription");
      }
    }
    ...
    const product = await stripe.products.create({
      ...
      description: "Monthly subscription - Personalized letters from Santa throughout the year",
      metadata: {
    ...
    console.error("❌ Error managing subscription product:", error);
...
// Helper function to get or create Stripe price for subscriptions
async function getOrCreateSubscriptionPrice(stripe: Stripe, productId: string, amountInCents: number): Promise<string> {
  try {
    // Try to retrieve the price from KV store
    const cachedPriceId = await kv.get("stripe:price:santa_subscription");
    if (cachedPriceId) {
      console.log("✅ Using cached price ID:", cachedPriceId);
      return cachedPriceId as string;
    }

    // Search for existing recurring price with same amount
    ...
    if (matchingPrice) {
      ...
      await kv.set("stripe:price:santa_subscription", matchingPrice.id);
    ...
    await kv.set("stripe:price:santa_subscription", price.id);
    return price.id;
  } catch (error) {
    console.error("❌ Error managing subscription price:", error);
    // Fallback: create inline without caching
    const price = await stripe.prices.create({
      product: productId,
      unit_amount: amountInCents,
...
async function logEmailActivity(
  ...
  try {
    ...
    if (!order) {
      console.error(`❌ Order ${orderId} not found for email logging`);
      return;
    }
    ...
    console.log(`✅ Email activity logged for order ${orderId}: ${emailType}`);
  } catch (error) {
    console.error(`❌ Error logging email activity for order ${orderId}:`, error);
  }
}
...
app.get("/make-server-cf244566/", (c) => {
  return c.json({ 
    message: "Santa Letters Server is running! 🎅",
    version: "2.0.6",
    timestamp: new Date().toISOString(),
...
app.get("/make-server-cf244566/track-click-test/:affiliateId", async (c) => {
  console.log("🧪 TEST ROUTE HIT: track-click-test");
...
app.get("/make-server-cf244566/track-click/:affiliateId", async (c) => {
  try {
    console.log("🎯 CLICK TRACKING ROUTE HIT");
    const affiliateId = c.req.param("affiliateId");
...
app.post("/make-server-cf244566/create-payment-intent", async (c) => {
  ...
  try {
    ...
    console.log("🔧 METADATA FIX VERSION 4 - Ultra minimal metadata");
    ...
    if (customerId) {
      ...
      console.log("🔗 Attaching customer to PaymentIntent:", customerId);
    } else {
      console.error("⚠️ No customer ID - this should not happen!");
    }
  ...
  try {
    ...
    if (paymentIntent.status !== 'succeeded') {
      ...
      try {
        ...
        console.log("💳❌ Declined payment logged:", declineId);
      } catch (logError) {
    ...
    if (monthlySubscription && stripe) {
      try {
        ...
        console.log("   - Monthly Subscription Flag:", monthlySubscription);
        console.log("   - Number of Letter Packages:", letterPackages.length);
        console.log("   - Payment Intent ID:", paymentIntentId);
        console.log("   - Billing Email:", billingData?.email);
        ...
        console.log("   - Trial End Date:", januaryFifth2026.toISOString());
        ...
        // If no customer, create one (fallback)
        if (!customerId) {
          console.log("⚠️ No customer attached to payment intent! Creating new customer...");
          const customer = await stripe.customers.create({
            email: billingData.email,
            name: `${billingData.firstName} ${billingData.lastName}`,
            phone: billingData.phone || "",
            metadata: {
              createdFrom: "subscription-fallback",
        ...
        if (!paymentMethodId) {
          console.error("❌ CRITICAL: No payment method on payment intent!");
        ...
        // Get payment method details to check if already attached
        let paymentMethod;
        try {
        ...
        if (paymentMethod.customer !== customerId) {
          ...
          try {
            ...
            console.error("   - Error code:", attachError.code);
            console.error("   - Error type:", attachError.type);
            ...
            if (attachError.code === 'resource_already_exists' || attachError.message?.includes('already been attached')) {
              console.log("✅ Payment method already attached (ignoring error)");
            } else {
              throw attachError;
            }
          }
        } else {
          console.log("✅ Payment method already attached to correct customer");
        }

        // Set as default payment method
        console.log("-".repeat(60));
        console.log("⚙️ Setting as default payment method...");
        try {
        ...
        console.log("-".repeat(60));
        console.log("📦 Getting subscription product...");
        ...
        console.log("   - Quantity:", subscriptionQuantity, "children");
        console.log("   - Unit Price: $12.00/month per child");
        console.log("   - Total: $" + (subscriptionQuantity * 12) + "/month");
        console.log("   - Trial Until:", januaryFifth2026.toLocaleDateString());
        ...
        console.log("=".repeat(60));
        console.log("   - Subscription ID:", createdSubscriptionId);
        console.log("   - Status:", subscription.status);
        console.log("   - Trial Until:", new Date(subscription.trial_end! * 1000).toLocaleDateString());
        console.log("   - Monthly Charge: $" + (subscriptionQuantity * 12) + " (" + subscriptionQuantity + " children × $12)");
        console.log("   - Stripe URL: https://dashboard.stripe.com/" + (createdSubscriptionId.startsWith('sub_test') ? 'test/' : '') + "subscriptions/" + createdSubscriptionId);
        console.log("=".repeat(60));
      } catch (error) {
        console.log("=".repeat(60));
        ...
        console.error("   - Customer ID:", paymentIntent.customer || "MISSING");
        console.error("   - Payment Method ID:", paymentIntent.payment_method || "MISSING");
        console.error("   - Monthly Subscription:", monthlySubscription);
        console.error("   - Number of Packages:", letterPackages.length);
        console.log("-".repeat(60));
      ...
      console.log("ℹ️ Skipping subscription creation:");
      console.log("   - monthlySubscription:", monthlySubscription);
    ...
    // Add subscription creation to activity log if applicable
    if (createdSubscriptionId) {
      const subscriptionUrl = createdSubscriptionId.startsWith('sub_test') 
        ? `https://dashboard.stripe.com/test/subscriptions/${createdSubscriptionId}`
        : `https://dashboard.stripe.com/subscriptions/${createdSubscriptionId}`;
      
      activityLog.push({
        timestamp: orderDate,
        action: 'Subscription Created',
        details: `Monthly subscription created for ${letterPackages.length} ${letterPackages.length === 1 ? 'child' : 'children'} ($${(letterPackages.length * 12).toFixed(2)}/month). First billing: January 5, 2026`,
        user: 'System',
        stripeUrl: subscriptionUrl
      });
    }
    ...
    const orderData = {
      ...
      stripeSubscriptionUrl: createdSubscriptionId 
        ? (createdSubscriptionId.startsWith('sub_test') 
            ? `https://dashboard.stripe.com/test/subscriptions/${createdSubscriptionId}`
            : `https://dashboard.stripe.com/subscriptions/${createdSubscriptionId}`)
        : undefined,
    ...
    console.log("🔐 Secure access token generated:", accessToken.substring(0, 20) + "...");
    ...
    if (affiliateId) {
      try {
        ...
        console.log("✅ Affiliate conversion tracked:", affiliateId);
      } catch (error) {
    ...
    try {
      ...
      if (resendApiKey && billingData.email) {
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Letter From Santa</h1>
              ...
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                ...
                <p style="font-size: 18px;"><span style="font-size: 24px;">🎉</span> Thank you for your order! We're excited to help you keep the magic of Christmas alive.</p>
                ...
                <ul style="line-height: 1.8;">
                  <li>✨ We're preparing your personalized letters from Santa</li>
                  <li>📝 Each letter will include a Good Behavior Certificate and Nice List</li>
                  <li>📮 Your packages will be mailed on your selected date: ${billingData.shippingDate}</li>
                  <li>📧 You'll receive a shipping notification with tracking</li>
                </ul>
                
                <p style="margin-top: 30px; text-align: center;">
                  <strong>📋 Manage Your Order Anytime:</strong><br>
                ...
                <p style="margin-top: 30px; padding: 20px; background: #fef2f2; border-radius: 8px; border-left: 4px solid #dc2626;">
                  <strong>🎄 Keep the Magic Alive!</strong><br>
                  Make sure to retrieve the mail before your little ones do! 😊
                </p>
              </div>
        ...
        console.log("📧 Attempting to send confirmation email to:", billingData.email);
        ...
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            subject: "🎅 Your Letter From Santa is On The Way!",
            html: emailHtml,
          }),
        });
        ...
        if (emailResponse.ok) {
          ...
          console.log("✅ Confirmation email sent successfully! Email ID:", emailData.id);
      ...
      if (twilioAccountSid && twilioAuthToken && twilioPhoneNumber && billingData.phone) {
        const smsBody = `🎅 Your Letter From Santa order is confirmed! Order #${orderId.slice(-8)}. Check your email for details and your order tracking link.`;
        
        console.log("📱 Attempting to send confirmation SMS to:", billingData.phone);
        ...
        if (smsResponse.ok) {
          ...
          console.log("✅ Confirmation SMS sent successfully! Message SID:", smsData.sid);
        } else {
          const errorText = await smsResponse.text();
          console.error("❌ Failed to send confirmation SMS. Status:", smsResponse.status);
          console.error("❌ Error details:", errorText);
        }
      } else {
        console.log("⚠️ SMS notification skipped. Twilio configured:", !!twilioAccountSid, "Phone provided:", !!billingData.phone);
      }
    } catch (error) {
...
app.get("/make-server-cf244566/declined-orders", async (c) => {
  try {
    console.log("💳❌ Fetching all declined payments...");
    
    // Get all declined payments from KV store
    const declines = await kv.getByPrefix("decline:");
    
    console.log(`✅ Found ${declines.length} declined payments`);
...
app.get("/make-server-cf244566/order/:identifier", async (c) => {
  try {
    ...
    console.log("📦 Fetching order with identifier:", identifier.substring(0, 20) + "...");
...
app.post("/make-server-cf244566/order/:identifier/update", async (c) => {
  try {
    ...
    console.log("✏️ Updating order with identifier:", identifier.substring(0, 20) + "...");
    ...
    if (packagesChanged) {
      await createAdminAlert(
        "order_updated",
        orderId,
        "📝 Order Modified - Packages Changed",
        {
      ...
      if (resendApiKey && existingOrder.customerInfo?.email) {
        ...
        const packagesHtml = letterPackages.map((pkg: any, idx: number) => `
          <div style="background: #fef2f2; border: 2px solid #fecaca; border-radius: 8px; padding: 15px; margin: 10px 0;">
            <p style="margin: 5px 0;"><strong>📦 Package ${idx + 1}</strong></p>
        ...
        const emailHtml = `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Order Updated!</h1>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Hi ${existingOrder.customerInfo.name || 'there'},</p>
                
                <p style="font-size: 18px; color: #059669;"><strong>✅ Your order has been successfully updated!</strong></p>
                ...
                <h3 style="color: #dc2626; margin-top: 30px;">📬 Updated Letter Packages:</h3>
                ${packagesHtml}
                ...
                <p style="margin-top: 30px;">Merry Christmas! ✨<br>The Santa Letters Team</p>
              </div>
        ...
        if (emailResponse.ok) {
          const emailData = await emailResponse.json();
          console.log("✅ Update notification email sent. Email ID:", emailData.id);
...
app.post("/make-server-cf244566/orders/:orderId/tracking", async (c) => {
  try {
    ...
    console.log("📦 Adding tracking to order:", orderId);
    ...
    await kv.set(`order:${orderId}`, updatedOrder);
    console.log("✅ Tracking added and order fulfilled:", orderId);
      ...
      if (resendApiKey && existingOrder.customerInfo?.email) {
        ...
        const emailHtml = `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Your Package Has Shipped!</h1>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Dear ${existingOrder.customerInfo.name || 'Valued Customer'},</p>
                
                <p style="font-size: 18px; color: #059669;"><strong>🎉 Exciting news! Your Letter From Santa is on its way!</strong></p>
                
                <p><strong>Order Number:</strong> #${orderId.slice(-8)}</p>
                
                <div style="background: #dbeafe; border-left: 4px solid #3b82f6; padding: 15px; margin: 20px 0;">
                  <p style="margin: 0; font-size: 16px;"><strong>📦 Tracking Number:</strong></p>
                ...
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0;">
                  <p style="margin: 0;"><strong>💡 Tip:</strong> Make sure someone is available to receive the package, or check if a signature is required!</p>
                </div>
        ...
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: EMAIL_FROM,
            to: existingOrder.customerInfo.email,
            subject: `🎅 Your Package Has Shipped! Tracking: ${trackingNumber}`,
            html: emailHtml,
          }),
        });
        ...
        if (emailResponse.ok) {
          const emailData = await emailResponse.json();
          console.log("✅ Tracking notification email sent. Email ID:", emailData.id);
    ...
    try {
      ...
      if (twilioAccountSid && twilioAuthToken && twilioPhoneNumber && existingOrder.customerInfo?.phone) {
        const smsBody = `🎅 Your Letter From Santa has shipped! Track it here: ${trackingUrl || 'Check your email for details'}. Order #${orderId.slice(-8)}`;
        ...
        if (smsResponse.ok) {
          ...
          console.error("❌ Failed to send tracking SMS");
        }
      }
    } catch (smsError) {
...
app.post("/make-server-cf244566/webhook", async (c) => {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      // Create order object
      console.log("📦 Creating order object for session:", session.id);
      ...
      if (stripe && session.payment_intent && stripeCustomerId) {
        try {
          ...
          if (paymentIntent.payment_method) {
            ...
            console.log("💳 Payment method ID saved:", order.stripePaymentMethodId);
          }
        } catch (pmError) {
          console.warn("⚠️ Could not retrieve payment method:", pmError);
      ...
      try {
        ...
        console.log("✅ Order saved to KV store with customer ID:", stripeCustomerId);
        ...
        await kv.set("orders:all", allOrderIds);
        console.log("✅ Order added to index. New count:", allOrderIds.length);
        
        // Store token -> orderId mapping for quick lookup
        await kv.set(`token:${accessToken}`, session.id);
        console.log("🔐 Secure access token mapping created");
        ...
        if (resendApiKey && customerEmail) {
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                  ...
                  <p><span style="font-size: 24px;">🎉</span> Thank you for your order! We're excited to help you keep the magic of Christmas alive.</p>
                  ...
                  <ul>
                    <li>✨ We're preparing your personalized letters from Santa</li>
                    <li>📝 Each letter will include a Good Behavior Certificate and Nice List</li>
                    <li>📮 Your packages will ship within 2-3 business days</li>
                    <li>📧 You'll receive a shipping notification with tracking</li>
                  </ul>
                  ${monthlySubscription ? `
                  <p style="margin-top: 30px; padding: 20px; background: #fff7ed; border-radius: 8px; border-left: 4px solid #f97316;">
                    <strong>🎁 Welcome to Santa's Magical Adventures!</strong><br>
          ...
          const emailResponse = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              ...
              to: [customerEmail],
              subject: `🎅 Your Letters From Santa Order Confirmed! #${session.id.slice(-8)}`,
              html: emailHtml,
            }),
          });
          ...
          if (twilioSid && twilioToken && twilioPhone && customerPhone) {
            ...
            formData.append("Body", `🎅 Thank you! Your Letters From Santa order #${session.id.slice(-8)} is confirmed. We'll send tracking info when it ships! Check your email for details.`);
    ...
    if (event.type === "invoice.payment_succeeded") {
      ...
      console.log("💳 Subscription payment succeeded:", invoice.id, "Amount:", invoice.amount_paid / 100);
      ...
      if (parentOrder) {
        ...
        console.log(`📦 [Webhook] Creating fulfillment order. Subscription letters count: ${(parentOrder.subscriptionLetters || []).length}`);
        console.log(`📦 [Webhook] Subscription letters data:`, JSON.stringify(parentOrder.subscriptionLetters));
        ...
        console.log(`✅ Created new order ${newOrderId} for subscription payment`);
          ...
          if (resendApiKey && parentOrder.customerInfo?.email) {
            const emailResponse = await fetch("https://api.resend.com/emails", {
              ...
              body: JSON.stringify({
                ...
                subject: `🎅 Your Month ${parentOrder.successfulCharges} Santa Letter is Coming!`,
                html: `
                  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2 style="color: #dc2626;">🎄 Ho Ho Ho!</h2>
                    ...
                    <p style="color: #059669;">Thank you for continuing this magical journey with us! 🎅✨</p>
                  </div>
                `,
              }),
            });
            ...
            if (emailResponse.ok) {
              const emailData = await emailResponse.json();
              console.log("✅ Sent subscription payment notification email. Email ID:", emailData.id);
    ...
    if (event.type === "invoice.payment_failed") {
      ...
      for (const orderId of allOrderIds) {
        ...
        if (order && order.subscriptionId === invoice.subscription) {
            ...
            if (resendApiKey && order.customerInfo?.email) {
              const emailResponse = await fetch("https://api.resend.com/emails", {
                ...
                body: JSON.stringify({
                  ...
                  subject: "⚠️ Santa Letter Subscription - Payment Issue",
                  html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                      ...
                      <p style="color: #6b7280;">🎄 Santa's Workshop</p>
                    </div>
                  `,
                }),
              });
              ...
              if (emailResponse.ok) {
                const emailData = await emailResponse.json();
                console.log("✅ Sent payment failed email. Email ID:", emailData.id);
          ...
          console.log(`✅ Updated order ${orderId} with declined charge`);
          break;
        }
      }
    }
...
app.post("/make-server-cf244566/test-webhook-simulation", async (c) => {
  ...
  try {
    ...
    console.log("✅ Webhook simulation complete - order created:", testSessionId);
...
app.get("/make-server-cf244566/order/:token", async (c) => {
  try {
    ...
    console.log(`🔍 Looking up order by token: ${token?.substring(0, 15)}...`);
    ...
    if (!token.startsWith("oat_")) {
      ...
      if (order) {
        console.log(`✅ Found order by direct ID (legacy): ${order.orderId}`);
      }
    }
    ...
    if (!order) {
      for (const orderId of allOrderIds) {
        ...
        if (orderData && orderData.accessToken === token) {
          order = orderData;
          console.log(`✅ Found order by access token: ${order.orderId}`);
          break;
        }
      }
    }
    ...
    if (!order) {
      console.error("❌ Order not found for token");
      return c.json({ error: "Order not found" }, 404);
    }
    
    // AUTO-FIX: If customer ID is missing, try to retrieve or create it
    if (!order.stripeCustomerId && stripe) {
      console.log("🔧 AUTO-FIXING: Missing customer ID for order:", order.orderId);
      try {
        let customerId = null;
        ...
        if (order.stripePaymentId) {
          try {
            ...
            console.log("⚠️ Could not retrieve payment intent:", piError.message);
          }
        }
        
        // If still no customer, create a new one from order data
        if (!customerId && order.customerInfo?.email) {
          console.log("🆕 Creating new Stripe customer for order:", order.orderId);
          const customer = await stripe.customers.create({
        ...
        if (customerId) {
          ...
          console.log("✅ AUTO-FIXED: Saved customer ID to order");
        } else {
          console.error("❌ Could not fix customer ID - no email address available");
        }
      } catch (error: any) {
...
// Get order by access token (POST /orders/get - for Trackdesk pixel)
app.post("/make-server-cf244566/orders/get", async (c) => {
  try {
    ...
    console.log(`🔍 Trackdesk lookup - Finding order by token: ${accessToken?.substring(0, 15)}...`);
    ...
    // Legacy support: try direct order ID lookup first
    if (!accessToken.startsWith("oat_")) {
      order = await kv.get(`order:${accessToken}`);
      if (order) {
        console.log(`✅ Found order by direct ID (legacy): ${order.orderId}`);
      }
    }
    ...
    if (!order) {
      for (const orderId of allOrderIds) {
        const orderData: any = await kv.get(`order:${orderId}`);
        if (orderData && orderData.accessToken === accessToken) {
          order = orderData;
          console.log(`✅ Found order by access token: ${order.orderId}, Total: $${order.totalAmount || order.total || 0}`);
          break;
        }
      }
    }
    ...
    // Return order with totalAmount field
    return c.json({ 
      order: {
        orderId: order.orderId,
    ...
    if (status === "canceled-refunded" && oldStatus !== "canceled-refunded") {
      try {
        ...
        if (resendApiKey && customerEmail) {
          ...
          if (emailResponse.ok) {
            const emailData = await emailResponse.json();
            console.log("✅ Sent order canceled email. Email ID:", emailData.id);
      ...
      try {
        ...
        if (resendApiKey && customerEmail) {
          ...
          const emailHtml = `
            ...
            <body style="font-family: Arial, sans-serif;">
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                <h1>✏️ Order Updated</h1>
                <p>Dear ${customerName},</p>
                <p>Your order <strong>#${orderId.slice(-8)}</strong> has been updated.</p>
                <h3>Changes Made:</h3>
                <ul>${changesHtml}</ul>
                <div style="background-color: #fef3c7; padding: 15px; border-radius: 5px; margin: 20px 0;">
                  <strong>⚠️ Important:</strong> Our team will review and confirm these changes before shipping your order.
                </div>
          ...
          const emailResponse = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: "Letters From Santa <noreply@updates.santascertifiedletter.com>",
              to: [customerEmail],
              subject: `✏️ Order Updated - #${orderId.slice(-8)}`,
              html: emailHtml,
            }),
          });
          ...
          if (emailResponse.ok) {
            const emailData = await emailResponse.json();
            console.log("✅ Sent customer edit notification email. Email ID:", emailData.id);
    ...
    if (isFulfilled) {
        ...
        if (resendApiKey && order.customerInfo?.email) {
          const emailResponse = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              ...
              html: `
                <h2>🎅 New Letter Package Order Confirmed!</h2>
                <p>Dear ${order.customerInfo.name},</p>
                ...
                <ul>
                  ...
                  ${addNorthPoleSnow ? '<li><strong>North Pole Snow:</strong> Included ❄️</li>' : ''}
                ...
                <p>Thank you for spreading more Christmas magic! 🎄</p>
              `,
            }),
          });
          ...
          if (emailResponse.ok) {
            const emailData = await emailResponse.json();
            console.log("✅ Sent new order confirmation email. Email ID:", emailData.id);
      ...
      try {
        ...
        if (resendApiKey && order.customerInfo?.email) {
          const emailResponse = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              ...
              html: `
                ...
                <p>Dear ${order.customerInfo.name},</p>
                <p>We've added ${numberOfPackages} letter package(s)${addNorthPoleSnow ? ' + Certified North Pole Snow ❄️' : ''} to your order!</p>
                ...
                <p>Thank you for adding more Christmas magic! 🎄</p>
              `,
            }),
          });
          ...
          if (emailResponse.ok) {
            const emailData = await emailResponse.json();
            console.log("✅ Sent packages added confirmation email. Email ID:", emailData.id);
  ...
  try {
    ...
    switch (emailType) {
      case "order_confirmation":
        subject = "🎅 Order Confirmation - Santa's Letters";
        ...
        emailHtml = `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Thank You!</h1>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Hi ${customerName},</p>
                <p style="font-size: 18px; color: #059669;"><strong>✅ Your order has been received!</strong></p>
                <div style="background: #dbeafe; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6;">
                  <p style="margin: 0;"><strong>Order #:</strong> ${orderId.slice(-8)}</p>
                ...
                ${packageDetails}
                <p style="margin-top: 30px;">The North Pole elves are preparing your magical letters! 🎄</p>
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center;">
                <h1 style="font-family: 'Pacifico', cursive;">📦 Your Order Has Shipped!</h1>
              </div>
              <div style="background: white; padding: 30px;">
                <p>Hi ${customerName},</p>
                <p>Great news! Your magical letters from Santa are on their way! 🎅✨</p>
      ...
      case "order_update":
        subject = "📝 Order Updated - Changes Confirmed";
        emailHtml = `
          <!DOCTYPE html>
          <html>
          <body style="font-family: Arial, sans-serif;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              ...
              <p>Thank you for your updates. Our elves are working on it! 🎅</p>
            </div>
          </body>
          </html>
        `;
      ...
      case "subscription_payment":
        subject = "🎅 Monthly Subscription Payment Confirmed";
        emailHtml = `
          <!DOCTYPE html>
          <html>
          <body style="font-family: Arial, sans-serif;">
      ...
      case "packages_added":
        subject = "🎅 Order Confirmed - Additional Packages";
        emailHtml = `
          <!DOCTYPE html>
          <html>
          <body style="font-family: Arial, sans-serif;">
...
app.get("/make-server-cf244566/declines", async (c) => {
  try {
    ...
    console.log(`📊 Fetched ${declines.length} declined orders`);
    ...
    for (const linkId of allLinkIds) {
      ...
      if (link) {
        ...
        if (fixedUrl !== link.trackingUrl) {
          ...
          console.log(`✅ Fixed tracking URL for link ${linkId}: ${fixedUrl}`);
        }
...
app.post("/make-server-cf244566/orders/:orderId/refund", async (c) => {
  try {
    ...
    if (order.status === "canceled-refunded" && order.refundAmount) {
      console.log(`⚠️ Order ${orderId} is already refunded: ${order.refundAmount}`);
      return c.json({ 
    ...
    try {
      ...
      if (stripeError.code === 'charge_already_refunded') {
        console.log(`⚠️ Stripe reports charge already refunded for order ${orderId}`);
    ...
    console.log(`✅ Refund processed successfully for order ${orderId}: ${amount}`);
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;"><h2 style="color: #dc2626;">🎅 Order Information Updated</h2><p>Hello ${order.customerInfo.name},</p><p>Your order information has been updated.</p><div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;"><p><strong>Order:</strong> #${order.orderId.slice(-8)}</p><p><strong>Updated:</strong> ${new Date().toLocaleDateString()}</p></div><p style="color: #dc2626;">🎄 Keep the magic alive!</p></div>`,
        }),
      });
    } catch (error) {
    ...
    try {
      ...
      await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilioAccountSid}/Messages.json`, {
        ...
        body: new URLSearchParams({
          ...
          From: twilioPhoneNumber,
          Body: `🎅 Your Santa letter order info has been updated. Order #${order.orderId.slice(-8)}`,
        }),
      });
    } catch (error) {
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;"><h2 style="color: #dc2626;">🎅 Subscription Canceled</h2><p>Hello ${order.customerInfo.name},</p><p>Your Santa's Magical Adventures subscription has been canceled.</p><div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;"><p><strong>Order:</strong> #${order.orderId.slice(-8)}</p><p><strong>Subscription ID:</strong> ${order.subscriptionId}</p></div><p>You will not be charged for any future months.</p><p style="color: #dc2626;">🎄 We hope to see you again next year!</p></div>`,
        }),
      });
    } catch (error) {
      console.error("❌ Failed to send email:", error);
    ...
    try {
      ...
      await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilioAccountSid}/Messages.json`, {
        ...
        body: new URLSearchParams({
          To: order.customerInfo.phone,
          From: twilioPhoneNumber,
          Body: `🎅 Your Santa subscription has been canceled. No future charges will be made.`,
        }),
      });
    } catch (error) {
      console.error("❌ Failed to send SMS:", error);
    ...
    try {
      ...
      await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilioAccountSid}/Messages.json`, {
        ...
        body: new URLSearchParams({
          To: order.customerInfo.phone,
          From: twilioPhoneNumber,
          Body: `💵 Refund of ${amount.toFixed(2)} processed for order #${order.orderId.slice(-8)}. Funds will appear in 5-10 business days.`,
        }),
      });
    } catch (error) {
      console.error("❌ Failed to send SMS:", error);
...
app.post("/make-server-cf244566/admin/update-billing-date", async (c) => {
  ...
  try {
    ...
    console.log(`📅 Updating next billing date for order: ${orderId} to ${nextBillingDate}`);
    ...
    if (nextBillingTimestamp <= now) {
      console.log(`🔔 Billing date is today or past - charging immediately`);
      ...
      if (paidInvoice.status !== 'paid') {
        console.error(`⚠️ Invoice not paid. Status: ${paidInvoice.status}`);
        return c.json({ error: `Payment failed. Invoice status: ${paidInvoice.status}` }, 400);
      }
      
      console.log(`✅ Immediate charge successful: ${(paidInvoice.amount_paid / 100).toFixed(2)}`);
      ...
      console.log(`✅ Created new order ${newOrderId} for manual subscription charge`);
          ...
          if (resendApiKey) {
            await fetch("https://api.resend.com/emails", {
              ...
              body: JSON.stringify({
                from: "Santa's Workshop <noreply@updates.santascertifiedletter.com>",
                to: [order.customerInfo.email],
                subject: "🎅 Your Monthly Santa Letter is On The Way!",
                html: `
                  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <p style="color: #dc2626;">🎄 Thank you for keeping the magic alive!</p>
                  </div>
                `,
              }),
            });
    ...
    console.log(`✅ Updated Stripe subscription billing anchor to: ${nextBillingDate}`);
    ...
    console.log(`✅ Updated order in database with new billing date: ${nextBillingDate}`);
...
app.post("/make-server-cf244566/admin/charge-subscription", async (c) => {
  ...
  try {
    ...
    console.log(`🔔 Manual billing requested for order: ${orderId}`);
    ...
    if (!skipRecentChargeCheck && order.lastChargeDate) {
      ...
      if (daysSinceLastCharge < 30) {
        console.warn(`⚠️ Order was charged ${daysSinceLastCharge.toFixed(1)} days ago - within 30 days`);
        return c.json({ 
          error: "RECENT_CHARGE",
          recentCharge: true,
    ...
    if (!subscription.default_payment_method && !customer.deleted && !customer.invoice_settings?.default_payment_method) {
      console.error(`⚠�� No default payment method found for customer ${subscription.customer}`);
      return c.json({ 
    ...
    console.log(`💳 Charging subscription: ${subscriptionPrice.toFixed(2)}`);
    ...
    console.log(`💳 Payment Intent created: ${paymentIntent.id}`);
    console.log(`💰 Payment status: ${paymentIntent.status}, Amount: ${(paymentIntent.amount / 100).toFixed(2)}`);
    
    // Check payment status
    if (paymentIntent.status !== 'succeeded') {
      console.error(`⚠️ Payment did not succeed. Status: ${paymentIntent.status}`);
    ...
    if (billingTimestamp > now) {
      ...
      console.log(`✅ Updated next billing date to: ${nextBillingDate}`);
    }
    ...
    // Create a new order for this month's fulfillment
    console.log(`📦 Creating fulfillment order. Subscription letters count: ${(order.subscriptionLetters || []).length}`);
    console.log(`📦 Subscription letters data:`, JSON.stringify(order.subscriptionLetters));
    ...
    console.log(`✅ Created new order ${newOrderId} for "Charge Now"`);
    
    console.log(`✅ Manual charge successful: ${paidInvoice.id}, Amount: ${(paidInvoice.amount_paid / 100).toFixed(2)}`);
...
app.post("/make-server-cf244566/admin/update-billing-interval", async (c) => {
  try {
    ...
    console.log(`🔄 Updating billing interval for order: ${orderId} to ${billingIntervalDays} days`);
    ...
    console.log(`✅ Billing interval updated to ${billingIntervalDays} days`);
...
app.post("/make-server-cf244566/admin/update-subscription-price", async (c) => {
  try {
    ...
    console.log(`💰 Updating subscription price for order: ${orderId} to ${subscriptionPrice}`);
    ...
    console.log(`✅ Subscription price updated to ${subscriptionPrice}`);
...
app.post("/make-server-cf244566/admin/update-subscription-product", async (c) => {
  try {
    ...
    console.log(`📦 Updating subscription product name for order: ${orderId}`);
    ...
    console.log(`✅ Subscription product name updated`);
...
app.get("/make-server-cf244566/declined-orders", async (c) => {
  try {
    console.log("📋 Fetching all declined orders...");
    ...
    console.log(`✅ Found ${declines.length} declined orders`);
...
app.post("/make-server-cf244566/log-decline", async (c) => {
  try {
    ...
    console.log("📝 Logging declined payment:", paymentIntentId);
    ...
    console.log("✅ Decline logged successfully:", declineId);
...
app.get("/make-server-cf244566/decline/track/open/:declineId/:emailNumber", async (c) => {
  try {
    ...
    console.log(`��� Email opened: ${declineId}, email #${emailNumber}`);
    ...
    if (!alreadyLogged) {
      ...
      await kv.set(`decline:${declineId}`, decline);
      console.log(`✅ Logged email open for decline ${declineId}`);
    }
...
app.get("/make-server-cf244566/decline/track/open/:declineId", async (c) => {
  try {
    ...
    console.log(`📧 Email opened (no email number): ${declineId}`);
...
app.get("/make-server-cf244566/decline/track/click/:declineId/:emailNumber", async (c) => {
  try {
    ...
    console.log(`🖱️ Email clicked: ${declineId}, email #${emailNumber}`);
    ...
    if (decline) {
      ...
      await kv.set(`decline:${declineId}`, decline);
      console.log(`✅ Logged email click for decline ${declineId}`);
    }
...
app.get("/make-server-cf244566/decline/track/click/:declineId", async (c) => {
  try {
    ...
    console.log(`🖱️ Email clicked (no email number): ${declineId}`);
...
app.get("/make-server-cf244566/decline/unsubscribe/:declineId", async (c) => {
  try {
    ...
    console.log(`🚫 Unsubscribe request for: ${declineId}`);
    ...
    console.log(`✅ Unsubscribed: ${declineId}`);
...
app.post("/make-server-cf244566/decline/mark-converted", async (c) => {
  try {
    ...
    console.log(`��� Marking decline as converted: ${declineId} -> ${orderId}`);
    ...
    console.log(`✅ Decline marked as converted: ${declineId}`);
    
    return c.json({ success: true });
  } catch (error: any) {
    console.error("❌ Error marking decline as converted:", error);
...
app.post("/make-server-cf244566/decline/send-recovery", async (c) => {
  try {
    ...
    console.log(`📧 Sending manual recovery email for: ${declineId}`);
    ...
    console.log(`📧 Tracking URLs - Pixel: ${trackingPixelUrl}, Click: ${trackingClickUrl}`);
    ...
    const emailHtml = `
      ...
      <body>
        <div class="container">
          <div class="header">
            <h1>🎅 Your Letter From Santa is Waiting!</h1>
          </div>
          <div class="content">
            ...
            <p style="font-size: 18px; color: #059669;"><strong>Don't worry - we've reserved your spot!</strong> 🎄</p>
    ...
    const emailResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `🎅 ${decline.customerInfo.name}, Your Santa Letters Are Reserved!`,
        html: emailHtml
      }),
    });
    ...
    console.log(`✅ Recovery email sent for decline ${declineId}`);
...
app.get("/make-server-cf244566/decline/unsubscribe/:declineId", async (c) => {
  try {
    ...
    console.log(`✅ Unsubscribed from decline emails: ${declineId}`);
    ...
    return c.html(`
      ...
      <body>
        ...
        <p style="color: #6b7280; font-size: 14px; margin-top: 30px;">
          Thank you for your interest in Letters From Santa! 🎅
        </p>
      </body>
      </html>
    `);
...
app.post("/make-server-cf244566/decline/process-sequence", async (c) => {
  try {
    console.log("🔄 Processing declined orders email sequence...");
    ...
    for (const decline of declines) {
      ...
      switch (dayNumber) {
        case 1:
          subject = "🎅 Complete Your Santa Letter Order";
        ...
        case 6:
          ...
          break;
        case 7:
          subject = "Final reminder: Santa Letters are waiting 🎅";
      ...
      const emailHtml = `
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
              <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Letters From Santa</h1>
            ...
            <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
              ...
              <div style="background: #fef2f2; border-left: 4px solid #dc2626; padding: 15px; margin: 20px 0;">
                <p style="margin: 0; font-size: 16px;"><strong>💳 What happened:</strong></p>
              ...
              <p style="margin-top: 30px; padding: 15px; background: #fef3c7; border-radius: 8px; border-left: 4px solid #f59e0b;">
                <strong>💡 Tip:</strong> Click "Update Payment Method" above to securely add a new card. Your order will be processed automatically once updated.
              </p>
      ...
      try {
        ...
        if (emailResponse.ok) {
          ...
          emailsSent++;
          console.log(`✅ Sent decline follow-up email #${dayNumber} to ${decline.customerInfo.email}`);
        } else {
          console.error(`❌ Failed to send decline email to ${decline.customerInfo.email}`);
        }
      } catch (emailError) {
    ...
    console.log(`📧 Email sequence complete: ${emailsSent} sent, ${emailsSkipped} skipped`);
...
app.get("/make-server-cf244566/decline/send-test-sequence", async (c) => {
  console.log("🧪 Manually triggering decline email sequence...");
...
app.post("/make-server-cf244566/subscriptions/:subscriptionId/create-order", async (c) => {
  try {
    ...
    console.log(`📦 Creating manual monthly order for subscription: ${subscriptionId}`);
    ...
    console.log(`✅ Created manual monthly order: ${newOrderId} for ${subscriptionProductName} at ${subscriptionPrice}`);
...
app.get("/make-server-cf244566/admin/declined-orders", async (c) => {
  try {
    console.log("📋 Fetching declined orders (admin alias)...");
    ...
    console.log(`✅ Found ${declinedOrders.length} declined orders`);
...
app.post("/make-server-cf244566/admin/process-retries", async (c) => {
  ...
  try {
    console.log("🔄 Processing declined charge retries...");
    ...
    for (const orderId of allOrderIds) {
      ...
      console.log(`🔁 Retrying charge for order ${orderId} (Attempt ${order.retryAttempts + 1})`);
      ...
      try {
        ...
        if (subscription.status === "canceled") {
          console.log(`⏭️ Subscription canceled, skipping retry`);
          continue;
        }
        ...
        if (paymentIntent.status === 'succeeded') {
          ...
          if (order.retryAttempts >= 3 && !order.downsellOffered) {
            ...
            console.log(`📧 Downsell offer sent for order ${orderId}`);
          } else {
            ...
            console.log(`❌ Retry failed for order ${orderId}, will retry again in 2 days`);
          }
        }
      } catch (error: any) {
        console.error(`❌ Error retrying charge for order ${orderId}:`, error);
        retriesFailed++;
    ...
    console.log(`✅ Retry processing complete: ${retriesAttempted} attempted, ${retriesSucceeded} succeeded, ${retriesFailed} failed, ${downsellOffersSent} downsell offers sent`);
...
app.post("/make-server-cf244566/accept-downsell/:orderId", async (c) => {
  ...
  try {
    ...
    console.log(`💰 Processing downsell acceptance for order ${orderId}: ${newPrice}`);
    ...
    try {
      ...
      if (paymentIntent.status === 'succeeded') {
        ...
        console.log(`✅ Downsell payment successful for order ${orderId}`);
      ...
      console.error("❌ Error charging downsell price:", error);
      return c.json({
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: linear-gradient(135deg, #dc2626 0%, #991b1b 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="margin: 0;">🎅 Payment Issue - Action Needed</h1>
      </div>
      <div style="background: #ffffff; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <div style="background: #fef3c7; padding: 20px; border-radius: 8px; border-left: 4px solid #f59e0b; margin: 20px 0;">
          <p style="margin: 0;"><strong>⚠️ We want to ship ${childNames}'s package!</strong></p>
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Letters From Santa <noreply@updates.santascertifiedletter.com>",
        to: [order.customerInfo.email],
        subject: `🎅 Payment Issue - ${childNames}'s Letter is Waiting!`,
        html: emailHtml,
      }),
    });
    console.log(`📧 Retry email sent to ${order.customerInfo.email}`);
  } catch (error) {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #ffffff; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <p>Great news! Your payment has been processed successfully and <strong>${childNames}'s</strong> monthly letter from Santa is being prepared! 🎅</p>
        ...
        <p style="color: #16a34a;"><strong>🎄 Thank you for keeping the Christmas spirit alive!</strong></p>
      </div>
    </div>
  `;
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Letters From Santa <noreply@updates.santascertifiedletter.com>",
        to: [order.customerInfo.email],
        subject: `🎅 Payment Received - ${childNames}'s Letter is On The Way!`,
        html: emailHtml,
      }),
    });
    console.log(`📧 Success email sent to ${order.customerInfo.email}`);
  } catch (error) {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="background: #ffffff; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <p>We really want ${childNames} to continue receiving magical letters from Santa! 🎅</p>
        ...
        <div style="background: #ede9fe; padding: 20px; border-radius: 8px; border-left: 4px solid #7c3aed; margin: 20px 0;">
          <p style="margin: 0;"><strong>💜 We'd like to offer you a special discount!</strong></p>
        </div>
        ...
        <p style="color: #7c3aed;"><strong>🎄 Keep the magic alive for ${childNames}!</strong></p>
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Letters From Santa <noreply@updates.santascertifiedletter.com>",
        to: [order.customerInfo.email],
        subject: `🎁 Special Offer: Discounted Letters from Santa for ${childNames}!`,
        html: emailHtml,
      }),
    });
    console.log(`📧 Downsell offer email sent to ${order.customerInfo.email}`);
  } catch (error) {
    console.error("❌ Failed to send downsell offer email:", error);
  }
}
  ...
  try {
    ...
    if (event.type === "invoice.payment_succeeded") {
      ...
      if (invoice.subscription) {
        console.log(`💳 Processing subscription payment: ${invoice.subscription}`);
        ...
        if (subscriptionOrder) {
          ...
          console.log(`📦 [Auto-Billing Webhook] Creating fulfillment order. Subscription letters count: ${(subscriptionOrder.subscriptionLetters || []).length}`);
          console.log(`📦 [Auto-Billing Webhook] Subscription letters data:`, JSON.stringify(subscriptionOrder.subscriptionLetters));
          ...
          console.log(`✅ Created automatic monthly order: ${newOrderId}`);
    ...
    if (event.type === "invoice.payment_failed") {
      ...
      if (invoice.subscription) {
        ...
        if (subscriptionOrder) {
          ...
          console.log(`📝 Tracked failed payment for order ${subscriptionOrder.orderId} - Attempt ${subscriptionOrder.retryAttempts}`);
    ...
    try {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Santa's Workshop <noreply@updates.santascertifiedletter.com>",
          to: [order.customerInfo.email],
          subject: "🎅 Your Monthly Letter from Santa is Coming!",
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Santa's Magical Adventures</h1>
                <p style="margin: 10px 0 0 0; font-size: 18px;">Your Monthly Letter is On Its Way!</p>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Ho ho ho! ${order.customerInfo.name},</p>
                
                <p style="font-size: 18px;"><span style="font-size: 24px;">🎉</span> Your monthly subscription has been renewed and your Santa letter is being prepared!</p>
                ...
                <ul style="line-height: 1.8;">
                  <li>✨ Santa is writing your personalized letter</li>
                  <li>📝 It will include updates on your good behavior and special activities</li>
                  <li>📮 Your letter will be mailed within the next few days</li>
                  <li>📧 You'll get tracking information once it ships</li>
                </ul>
                
                <p style="margin-top: 30px; text-align: center;">
                  <span style="font-size: 24px;">🎄</span> <strong>Keep up the great work!</strong> <span style="font-size: 24px;">🎄</span>
                </p>
              </div>
            </div>
          `,
      ...
      console.error("❌ Failed to send monthly notification email:", error);
    }
  }
    ...
    try {
      ...
      await fetch(`https://api.twilio.com/2010-04-01/Accounts/${twilioAccountSid}/Messages.json`, {
        ...
        body: new URLSearchParams({
          To: order.customerInfo.phone,
          From: twilioPhoneNumber,
          Body: `🎅 Ho ho ho! Your monthly Santa letter has been renewed and is being prepared. Order #${order.orderId.slice(-8)} - Month ${order.subscriptionMonthsActive}`,
        }),
      });
    } catch (error) {
      console.error("❌ Failed to send monthly notification SMS:", error);
    }
  }
}
...
app.get("/make-server-cf244566/get-order-for-update/:token", async (c) => {
  try {
    const token = c.req.param("token");
    console.log(`🔍 Looking up order for payment update with token: ${token}`);
    ...
    if (token.startsWith("decline_")) {
      ...
      if (declineData) {
        console.log(`✅ Found decline record: ${token}`);
    ...
    console.log(`✅ Found order: ${orderData.orderId}`);
...
app.post("/make-server-cf244566/update-payment-method", async (c) => {
  ...
  try {
    ...
    console.log(`💳 Updating payment method for token: ${token}`);
    console.log(`📝 Updated order data received:`, updatedOrderData ? 'Yes' : 'No');
    ...
    if (updatedOrderData) {
      console.log(`📝 Applying order updates...`);
      ...
      if (orderData) {
        ...
        console.log(`✅ Updated order data saved with total: ${orderData.total}`);
      }
      if (declineData && isDecline) {
        await kv.set(`decline:${token}`, declineData);
        console.log(`✅ Updated decline data saved with amount: ${declineData.amount}`);
      }
    }
        ...
        if (resendApiKey && emailTo) {
          const emailHtml = `
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                  <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">✅ Payment Method Updated!</h1>
                </div>
                <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                  <p>Hi ${customerName || 'there'},</p>
                  
                  <p style="font-size: 18px; color: #059669;"><strong>✅ Your payment method has been successfully updated!</strong></p>
                  
                  <div style="background: #dcfce7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #16a34a;">
                    <p style="margin: 0 0 10px 0;"><strong>💳 New Payment Method:</strong></p>
                  ...
                  ${orderData?.monthlySubscription || declineData?.subscriptionId ? `
                    <div style="background: #dbeafe; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6;">
                      <p style="margin: 0 0 10px 0;"><strong>🎅 Subscription Active</strong></p>
                  ...
                  <div style="background: #fef2f2; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #dc2626;">
                    <p style="margin: 0; font-size: 13px;">
                      <strong>🔒 Security:</strong> For your protection, we never store your full card number. Only the last 4 digits are kept for reference.
                    </p>
                  </div>
          ...
          if (response.ok) {
            console.log("✅ Payment method update email sent to:", emailTo);
          } else {
            const errorText = await response.text();
            console.error("❌ Failed to send payment update email:", errorText);
          }
        }
      } catch (emailError) {
    ...
    if (paymentIntent.status === "requires_payment_method" || paymentIntent.status === "canceled") {
      ...
      console.log(`❌ Decline reason: ${errorMessage}`);
...
app.post("/make-server-cf244566/confirm-payment-update", async (c) => {
  ...
  try {
    ...
    console.log(`✅ Confirming payment update: ${paymentIntentId}`);
...
async function createOrderFromPaymentUpdate(
  ...
  console.log(`✅ Created order from payment update: ${orderId}`);
...
async function sendPaymentUpdateConfirmationEmail(order: any) {
  ...
  const emailHtml = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
          <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Payment Updated Successfully!</h1>
        </div>
        <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
          <p>Hi ${order.customerInfo.name},</p>
          
          <p style="font-size: 18px; color: #059669;"><strong>✅ Your payment method has been updated and your order is being processed!</strong></p>
          ...
          ${order.monthlySubscription ? `
            <div style="background: #dbeafe; border-left: 4px solid #3b82f6; padding: 15px; margin: 20px 0;">
              <p style="margin: 0; font-size: 16px;"><strong>📅 Monthly Subscription Active</strong></p>
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: EMAIL_FROM,
        to: [order.customerInfo.email],
        subject: "🎅 Payment Updated - Your Santa Letters Are On The Way!",
        html: emailHtml,
      }),
    });
    console.log(`📧 Payment update confirmation email sent to ${order.customerInfo.email}`);
  } catch (error) {
  ...
  try {
    ...
    if (allUpsellIds.length === 0) {
      ...
      const subscriptionUpsell = {
        ...
        active: true,
        isSubscription: true,
        promoText: "🎅 SPECIAL OFFER: Continue the magic all year long!",
        features: [
      ...
      console.log("✅ Created default upsells: North Pole Snow + Santa's Magical Journey");
...
app.post("/make-server-cf244566/admin/upsell/save", async (c) => {
  try {
    ...
    console.log(`✅ Upsell saved: ${upsellData.id}`, upsellData);
...
app.get("/make-server-cf244566/upsell/get-offer/:token", async (c) => {
  try {
    ...
    for (const upsell of activeUpsells) {
      ...
      if (upsell.isSubscription) {
        ...
        if (order.monthlySubscription && order.subscriptionId) {
          console.log(`⏭️ Skipping subscription upsell - customer already subscribed on checkout`);
          continue;
        }
      }
    ...
    console.log(`📦 Offering upsell ${nextUpsell.id} to order ${order.orderId} (Attempt ${attemptNumber}, Price: ${currentPrice})`);
    ...
    console.error("❌ Error getting upsell offer:", error);
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-cf244566/upsell/accept", async (c) => {
  try {
    ...
    console.log(`💳 Processing upsell: ${totalAmount.toFixed(2)} for ${quantity}x ${upsell.name} (${upsell.isSubscription ? 'Subscription' : 'One-time'})`);
    ...
    if (!order.stripePaymentMethodId) {
      console.log("⚠️ No payment method stored, retrieving from Stripe customer...");
      try {
        ...
        if (!defaultPaymentMethod) {
          ...
          if (paymentMethods.data.length > 0) {
            ...
            console.log("✅ Found payment method from customer:", order.stripePaymentMethodId);
          ...
          console.log("✅ Retrieved default payment method:", order.stripePaymentMethodId);
          // Save it back to order for future use
          await kv.set(`order:${order.orderId}`, order);
        }
      } catch (error: any) {
    ...
    if (upsell.isSubscription) {
      ...
      console.log(`📅 Creating subscription at ${actualPrice}/month for ${quantity} kid(s)`);
      ...
      if (paymentIntent.status !== 'succeeded') {
        console.error(`❌ Subscription payment failed: ${paymentIntent.status}`);
      ...
      if (paymentIntent.status !== 'succeeded') {
        console.error(`❌ Upsell payment failed: ${paymentIntent.status}`);
        return c.json({ 
          error: "Payment was declined. Please contact support.",
          declineCode: paymentIntent.last_payment_error?.code
        }, 402);
    ...
    console.log(`✅ Upsell accepted and charged: ${upsellId}`);
      ...
      if (resendApiKey && order.customerInfo?.email) {
        const emailHtml = `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎁 ${upsell.isSubscription ? 'Subscription Started!' : 'Item Added to Your Order!'}</h1>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Hi ${order.customerInfo.name || 'there'},</p>
                
                <p style="font-size: 18px; color: #059669;"><strong>✅ Great choice! Your order has been updated.</strong></p>
                ...
                <div style="background: #dcfce7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #16a34a;">
                  ...
                  ${upsell.isSubscription ? `
                    <p style="margin: 10px 0 0 0; font-size: 14px; color: #166534;">
                      💳 Monthly subscription at ${actualPrice.toFixed(2)}/month per child<br>
                      📅 First payment charged today: ${totalAmount.toFixed(2)}<br>
                ...
                ${upsell.isSubscription ? `
                  <div style="background: #dbeafe; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6;">
                    <p style="margin: 0 0 10px 0;"><strong>🎅 Welcome to Santa's Magical Journey!</strong></p>
                ...
                <p style="margin-top: 30px;">Thank you for your purchase! The magic continues... ✨</p>
              </div>
              <div style="background: #f9fafb; padding: 20px; text-align: center; color: #6b7280; font-size: 14px; border-radius: 0 0 10px 10px;">
                <p>Questions? Contact us at support@letterfromsanta.com</p>
        ...
        const response = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            subject: upsell.isSubscription 
              ? `🎅 Subscription Started - Santa's Magical Journey!`
              : `🎁 Item Added to Your Order - ${upsell.name}`,
            html: emailHtml,
          }),
        });

        if (response.ok) {
          console.log("✅ Upsell confirmation email sent to:", order.customerInfo.email);
        } else {
          const errorText = await response.text();
          console.error("�� Failed to send upsell email:", errorText);
        }
      }
    } catch (emailError) {
      console.error("❌ Error sending upsell confirmation email:", emailError);
    }
...
app.post("/make-server-cf244566/upsell/accept-snow", async (c) => {
  try {
    ...
    try {
      ...
      console.log(`✅ Snow upsell accepted: Order ${order.orderId}, Quantity: ${quantity}, Amount: ${totalAmount}`);
      ...
      console.error("❌ Error charging for snow upsell:", error);
    ...
    console.error("❌ Error accepting snow upsell:", error);
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-cf244566/upsell/accept-snow-downsell", async (c) => {
  try {
    ...
    try {
      ...
      console.log(`✅ Snow downsell accepted: Order ${order.orderId}, Quantity: ${quantity}, Amount: ${totalAmount}`);
      ...
      console.error("❌ Error charging for snow downsell:", error);
    ...
    console.error("❌ Error accepting snow downsell:", error);
    return c.json({ error: error.message }, 500);
  }
});

// Accept subscription upsell (funnel - $12/month per child)
app.post("/make-server-cf244566/upsell/accept-subscription", async (c) => {
  try {
    const { orderToken, quantity } = await c.req.json();
    ...
    const pricePerChild = 12.00; // $12 per child per month
    const totalAmount = 0; // FREE today - First charge in January
    
    try {
      console.log(`🔄 Creating subscription for ${quantity} child(ren) - ${order.orderId}`);
      
      // Get or create subscription product
      const productId = await getOrCreateSubscriptionProduct(stripe);
      
      // Get or create subscription price
      const priceId = await getOrCreateSubscriptionPrice(stripe, productId, pricePerChild * 100);
      
      // Create subscription with $0 trial until January 1st
      const today = new Date();
      const nextJanuary = new Date(today.getFullYear() + 1, 0, 1); // January 1st next year
      const trialEndTimestamp = Math.floor(nextJanuary.getTime() / 1000);
      ...
      order.subscriptions.push({
        id: subscription.id,
        name: "Santa's Magical Journey",
        pricePerChild: pricePerChild,
        quantity: quantity,
        monthlyTotal: pricePerChild * quantity,
        status: 'trialing',
        trialEnd: nextJanuary.toISOString(),
        firstChargeDate: 'January 1st',
        acceptedAt: new Date().toISOString(),
        source: 'funnel_upsell'
      });
      
      order.hasSubscription = true;
      order.updatedAt = new Date().toISOString();
      ...
      order.activityLog.push({
        ...
        details: `Santa's Magical Journey - ${quantity} child(ren) x $${pricePerChild}/month - FREE until January 1st`,
        user: 'Customer',
      ...
      console.log(`✅ Subscription added to order: ${order.orderId}, Quantity: ${quantity}`);
    ...
    console.error("❌ Error accepting subscription upsell:", error);
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-cf244566/order/:token/create-upsell-payment", async (c) => {
  try {
    ...
    console.log("📦 Order found for upsell payment:", {
      orderId: order.orderId,
    ...
    if (!order.stripeCustomerId) {
      console.error("❌ Order still missing customer ID after auto-fix. This shouldn't happen but continuing anyway:", order.orderId);
      // Don't return error - let Stripe handle it when creating payment intent
    }
    
    const totalAmount = price * quantity;
    
    console.log(`💳 Creating payment intent for customer: ${order.stripeCustomerId}`);
    ...
    console.log(`✅ Created payment intent for upsell: ${paymentIntent.id}`);
    
    return c.json({ clientSecret: paymentIntent.client_secret });
  } catch (error: any) {
    console.error("❌ Error creating upsell payment intent:", error);
...
app.post("/make-server-cf244566/order/:token/confirm-upsell", async (c) => {
  try {
    ...
    console.log(`✅ Upsell confirmed for order ${order.orderId}: ${upsellName} x${quantity}`);
      ...
      if (resendApiKey) {
        await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: EMAIL_FROM,
            to: order.customerInfo.email,
            subject: `🎁 ${upsellName} Added to Your Order!`,
            html: `
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <h1 style="color: #c41e3a;">✅ Purchase Confirmed!</h1>
...
app.post("/make-server-cf244566/order/:token/add-upsell", async (c) => {
  try {
    ...
    console.log(`✅ Upsell added from success page: ${upsellName} x${quantity}`);
      ...
      if (resendApiKey && order.customerInfo?.email) {
        const emailHtml = `
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎁 Item Added!</h1>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Hi ${order.customerInfo.name || 'there'},</p>
                
                <p style="font-size: 18px; color: #059669;"><strong>✅ We're adding ${upsellName} to your order!</strong></p>
                ...
                <div style="background: #dbeafe; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #3b82f6;">
                  <p style="margin: 0; font-size: 14px; color: #1e40af;">
                    <strong>✨ Your ${upsellName} will be shipped with your letter package!</strong>
                  </p>
                </div>
                
                <p style="margin-top: 30px;">Thank you for your purchase! The magic continues... ✨</p>
                
                <p>With Christmas cheer,<br>The Santa's Letters Team 🎄</p>
              </div>
              <div style="background: #f9fafb; padding: 20px; text-align: center; color: #6b7280; font-size: 14px; border-radius: 0 0 10px 10px;">
                <p>Questions? Contact us at support@letterfromsanta.com</p>
                <p style="margin: 5px 0 0 0;">© ${new Date().getFullYear()} Letter From Santa. All rights reserved.</p>
    ...
    console.error("❌ Error adding upsell from success page:", error);
...
app.post("/make-server-cf244566/upsell/decline", async (c) => {
  try {
    ...
    const attempts = order.upsellDeclineAttempts[upsellId] || 0;
    console.log(`👎 Upsell declined: ${upsellId} (Attempt ${attempts}/2)`);
    ...
    console.error("❌ Error declining upsell:", error);
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-cf244566/upsell/complete", async (c) => {
  try {
    ...
    // DISABLED: Don't send confirmation email here - customer already received initial order email
    // await sendOrderConfirmationWithUpsells(order);
    
    console.log(`✅ Upsell flow completed for order: ${order.orderId}`);
...
async function sendOrderConfirmationWithUpsells(order: any) {
  ...
  if (hasUpsells) {
    upsellsHtml = `
      <div style="background: #f0fdf4; border: 2px solid #059669; border-radius: 8px; padding: 20px; margin: 20px 0;">
        <h3 style="margin-top: 0; color: #059669; font-family: 'Pacifico', cursive;">🎁 Bonus Items Added</h3>
  ...
  const emailHtml = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
          <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Order Confirmed!</h1>
        </div>
        <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
          <p>Hi ${order.customerInfo.name},</p>
          
          <p style="font-size: 18px; color: #059669;"><strong>✅ Your order has been confirmed and is being processed!</strong></p>
          ...
          ${order.monthlySubscription ? `
          <div style="background: #dbeafe; border-left: 4px solid #3b82f6; padding: 15px; margin: 20px 0;">
            <p style="margin: 0;"><strong>📅 Monthly Subscription Active</strong></p>
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: EMAIL_FROM,
        to: order.customerInfo.email,
        subject: `🎅 Order Confirmed - Santa's Letters`,
        html: emailHtml,
      }),
    });
...
async function createAdminAlert(
  ...
  try {
    ...
    console.log(`✅ Admin alert created: ${type} for order ${orderId}`);
  } catch (error) {
...
app.post("/make-server-cf244566/stripe-webhook", async (c) => {
  ...
  try {
    ...
    if (event.type === "invoice.payment_failed") {
      ...
      await kv.set(`decline:${declineId}`, declineData);
      console.log(`✅ Decline tracked: ${declineId}`);
...
app.post("/make-server-cf244566/admin/process-retries", async (c) => {
  ...
  try {
    ...
    for (const decline of allDeclines) {
      ...
      console.log(`🔄 Processing retry for decline ${declineData.declineId}`);
      retriesAttempted++;

      // Get the order
      const order = await kv.get(`order:${declineData.orderId}`);
      if (!order) {
        console.error(`❌ Order not found: ${declineData.orderId}`);
        continue;
      }
      ...
      try {
        ...
        if (!subscription.latest_invoice) {
          console.error(`❌ No invoice found for subscription ${declineData.subscriptionId}`);
          retriesFailed++;
          continue;
        }
        ...
        if (paidInvoice.status === "paid") {
          console.log(`✅ Retry successful for ${declineData.declineId}`);
          retriesSucceeded++;
        ...
        console.error(`❌ Retry failed for ${declineData.declineId}:`, error.message);
        retriesFailed++;
...
async function sendRecoveryEmail(order: any, decline: any) {
  const resendApiKey = Deno.env.get("RESEND_API_KEY");
  if (!resendApiKey || !order.customerInfo?.email) {
    console.log("⚠️ Cannot send recovery email - missing API key or email");
    return;
  }
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #dc2626 0%, #ea580c 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
          <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎅 Payment Issue</h1>
        </div>
        <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
          <p>Hi ${order.customerInfo.name},</p>
          
          <p style="font-size: 18px; color: #dc2626;"><strong>⚠️ ${childNames}'s Letter is Waiting!</strong></p>
  ...
  try {
    ...
    if (!response.ok) {
      console.error("❌ Failed to send recovery email:", await response.text());
    } else {
      console.log(`✅ Recovery email sent to ${order.customerInfo.email}`);
    }
  } catch (error) {
    console.error("❌ Error sending recovery email:", error);
  }
...
app.post("/make-server-cf244566/admin/migrate-customer-ids", async (c) => {
  try {
    ...
    console.log(`🔄 Starting migration for ${allOrderIds.length} orders...`);
      ...
      if (order.stripePaymentId) {
        try {
          ...
          if (paymentIntent.customer) {
            ...
            await kv.set(`order:${orderId}`, order);
            updated++;
            console.log(`✅ Updated order ${orderId} with customer ID`);
          } else {
            skipped++;
            console.log(`⏭️ Order ${orderId} has no customer in payment intent`);
          }
        } catch (error: any) {
          errors++;
...
app.post("/make-server-cf244566/decline/retry-payment", async (c) => {
  ...
  try {
    ...
    console.log(`🔄 Admin retry payment: ${paymentIntentId}`);
    ...
    if (result.status === 'succeeded') {
      ...
      console.log(`✅ Payment retry successful: ${result.id}`);
      return c.json({ success: true, paymentIntentId: result.id });
    } else {
      console.log(`❌ Payment retry failed: ${result.status}`);
      return c.json({ 
...
app.post("/make-server-cf244566/decline/update-payment-method", async (c) => {
  ...
  try {
    ...
    console.log(`💳 Admin updating payment method for: ${paymentIntentId}`);
...
app.post("/make-server-cf244566/decline/stop-recovery", async (c) => {
  try {
    ...
    console.log(`🛑 Admin stopping recovery for: ${declineId}`);
    ...
    console.log(`✅ Recovery stopped for: ${declineId}`);
    return c.json({ success: true });
  } catch (error: any) {
    console.error("❌ Stop recovery error:", error);
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-cf244566/order/:token/create-upsell-payment", async (c) => {
  ...
  try {
    ...
    console.log(`💳 Creating payment intent for upsell: ${upsellName} x${quantity}`);
...
app.post("/make-server-cf244566/order/:token/confirm-upsell", async (c) => {
  ...
  try {
    ...
    console.log(`✅ Confirming upsell payment: ${paymentIntentId}`);
    ...
    if (upsellId === "upsell_subscription") {
      console.log(`🎅 Creating subscription from success page for ${qty} children`);
      ...
      console.log(`📅 Setting trial_end to January 1st: ${january1st.toISOString()}`);
      ...
      try {
        ...
        if (!subscriptionPriceId) {
          console.error('❌ STRIPE_SUBSCRIPTION_PRICE_ID environment variable not set!');
        ...
        console.log(`✅ Subscription created: ${subscription.id} for ${qty} children, billing starts January 1st`);
        ...
        console.log(`✅ Created separate subscription order: ${subscriptionOrderId}`);
        ...
        console.log(`✅ Original order ${orderId} updated with subscription reference`);
        ...
        console.error("❌ Failed to create subscription from success page:", subError);
      ...
      console.log(`✅ Upsell confirmed and added to order: ${upsellName}`);
        ...
        if (resendApiKey && order.customerInfo?.email) {
          const emailHtml = `
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
              <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                <div style="background: linear-gradient(135deg, #dc2626 0%, #059669 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                  <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">🎁 Item Added to Your Order!</h1>
                </div>
                <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                  <p>Hi ${order.customerInfo.name || "there"},</p>
          ...
          console.log("📧 Upsell confirmation email sent");
        }
      } catch (emailError) {
...
app.post("/make-server-cf244566/admin/update-subscription-letters", async (c) => {
  try {
    ...
    console.log(`✅ Updated subscription letters for ${orderId}`);
    return c.json({ success: true });
  } catch (error: any) {
    console.error("❌ Error updating subscription letters:", error);
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-cf244566/admin/update-subscription-quantity", async (c) => {
  ...
  try {
    ...
    if (subscriptionId && stripe) {
      try {
        ...
        if (subscription.items.data.length > 0) {
          ...
          console.log(`✅ Updated Stripe subscription quantity from ${oldQuantity} to ${newQuantity} (Price: ${oldPrice.toFixed(2)} → ${newPrice.toFixed(2)})`);
        }
      } catch (stripeError) {
    ...
    console.log(`✅ Updated subscription quantity for ${orderId}: ${oldQuantity} → ${newQuantity} (Price: ${oldPrice.toFixed(2)} → ${newPrice.toFixed(2)})`);
    return c.json({ success: true });
  } catch (error: any) {
...
app.post("/make-server-cf244566/admin/charge-subscription-now", async (c) => {
  ...
  try {
    ...
    console.log(`✅ Successfully charged ${amount} for subscription ${orderId}`);
    ...
    console.log(`📦 [Custom Charge] Creating fulfillment order. Subscription letters count: ${(order.subscriptionLetters || []).length}`);
    console.log(`📦 [Custom Charge] Subscription letters data:`, JSON.stringify(order.subscriptionLetters));
    ...
    console.log(`✅ Created new order ${newOrderId} for manual charge`);
      ...
      if (resendApiKey && order.customerInfo?.email) {
          ...
          <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; margin: 0; padding: 0;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #7c3aed 0%, #db2777 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                <h1 style="margin: 0; font-family: 'Pacifico', cursive; font-size: 32px;">💳 Payment Processed</h1>
              </div>
              <div style="background: #ffffff; padding: 30px; border: 2px solid #e5e7eb; border-top: none;">
                <p>Hi ${order.customerInfo.name || "there"},</p>
        ...
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: EMAIL_FROM,
            to: [order.customerInfo.email],
            subject: `💳 Subscription Payment Processed - ${amount.toFixed(2)}`,
            html: emailHtml,
          }),
        });
        ...
        if (emailResponse.ok) {
          const emailData = await emailResponse.json();
          console.log(`✅ Payment confirmation email sent to ${order.customerInfo.email}`);
          ...
          await logEmailActivity(
            ...
            order.customerInfo.email,
            `💳 Subscription Payment Processed - ${amount.toFixed(2)}`,
            emailData.id
          );
        } else {
          const errorText = await emailResponse.text();
...
app.post("/make-server-cf244566/admin/update-settings", async (c) => {
  try {
    ...
    console.log(`✅ Updated global settings: defaultLetterPrice = ${defaultLetterPrice}`);
...
app.get("/make-server-cf244566/debug/database", async (c) => {
  try {
    const allOrderIds = (await kv.get("orders:all")) || [];
    
    console.log("🔍 DATABASE DEBUG:");
    console.log("📋 orders:all key contains:", allOrderIds);
    console.log("📊 Total order IDs:", allOrderIds.length);
    
    // Sample first few orders
    const sampleOrders = [];
    for (let i = 0; i < Math.min(3, allOrderIds.length); i++) {
      const order = await kv.get(`order:${allOrderIds[i]}`);
      sampleOrders.push({
        orderId: allOrderIds[i],
        found: !!order,
        orderDate: order?.orderDate,
        total: order?.total
      });
    ...
    return c.json({
      ...
      message: allOrderIds.length === 0 ? "No orders found - database is empty. Create a test order." : "Orders found in database"
    });
  } catch (error: any) {
...
app.post("/make-server-cf244566/debug/create-test-order", async (c) => {
  try {
    const testOrderId = `test_order_${Date.now()}`;
    const testOrder = {
      orderId: testOrderId,
      ...
      letterPackages: [{
        packageType: "Gold Edition",
        childName: "Test Child",
        childAge: "5",
        price: 29.99
      }],
      numberOfPackages: 1,
      shippingDate: "December 10th",
      monthlySubscription: false,
      paymentIntentId: "test_pi_" + Date.now(),
      activityLog: [{
        timestamp: new Date().toISOString(),
        action: "Test Order Created",
        details: "This is a test order created for debugging",
        user: "Debug Tool"
      }]
    };
    ...
    const verifyOrderIds = (await kv.get("orders:all")) || [];
    const verifyOrder = await kv.get(`order:${testOrderId}`);
    console.log("✅ Verification:");
    console.log("   - Order list now has:", verifyOrderIds.length, "orders");
    console.log("   - New order exists:", !!verifyOrder);
    console.log("   - Order ID in list:", verifyOrderIds.includes(testOrderId));
    
    return c.json({
      success: true,
      orderId: testOrderId,
      message: "Test order created successfully! Check admin dashboard.",
      totalOrders: allOrderIds.length,
      verification: {
        orderSaved: !!verifyOrder,
        listUpdated: verifyOrderIds.includes(testOrderId),
        totalInDatabase: verifyOrderIds.length
      }
    });
  } catch (error: any) {
    console.error("❌ Error creating test order:", error);
...
// =====================================================
// EMAIL SUPPRESSION / OPT-OUT SYSTEM
// =====================================================

// Unsubscribe from all emails
app.post("/make-server-cf244566/email/unsubscribe", async (c) => {
  try {
    const body = await c.req.json();
    ...
    // Store in suppression list
    await kv.set(`email:suppression:${normalizedEmail}`, {
    ...
    // Add to global suppression list for easy querying
    const suppressionList = (await kv.get("email:suppression:all")) || [];
    if (!suppressionList.includes(normalizedEmail)) {
      suppressionList.push(normalizedEmail);
      await kv.set("email:suppression:all", suppressionList);
    }
...
app.get("/make-server-cf244566/email/check-suppression/:email", async (c) => {
  try {
    const email = c.req.param("email");
    const normalizedEmail = email.toLowerCase().trim();
    
    const suppressionData = await kv.get(`email:suppression:${normalizedEmail}`);
...
// Get all suppressed emails (admin only)
app.get("/make-server-cf244566/email/suppression-list", async (c) => {
  try {
    const suppressionList = (await kv.get("email:suppression:all")) || [];
...
// Re-subscribe email (remove from suppression list)
app.post("/make-server-cf244566/email/resubscribe", async (c) => {
  try {
    const body = await c.req.json();
    const { email } = body;
    ...
    await kv.del(`email:suppression:${normalizedEmail}`);

    // Remove from global list
    const suppressionList = (await kv.get("email:suppression:all")) || [];
    const updatedList = suppressionList.filter((e: string) => e !== normalizedEmail);
    await kv.set("email:suppression:all", updatedList);

    console.log(`✅ Email re-subscribed: ${normalizedEmail}`);

    return c.json({ 
      success: true, 
      message: "Successfully re-subscribed to emails" 
    });
  } catch (error: any) {
    console.error("❌ Error re-subscribing email:", error);
    return c.json({ error: error.message }, 500);
  }
});
